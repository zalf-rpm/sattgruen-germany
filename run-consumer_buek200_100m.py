        ...
        if is_data_row:
            ...
            if not os.path.isfile(path_to_row_file):
                with open(path_to_row_file, "w") as _:
                    _.write("Year,row,col,AbBiom-max,Precip-sum,SumExportedCutBiomass-last,ShootBiom-max,LeafBiom-max,yearly-sum-nleach\n")
...
def run_consumer(leave_after_finished_run = True, server = {"server": None, "port": None}, shared_id = None):
    ...
    grid_template[grid_template != -9999] = 1
    #set all no-data values to 0, to ignore them while counting
    grid_template[grid_template == -9999] = 0
    #count cols in rows
    datacells_per_row = np.sum(grid_template, axis=1)

    start_row = int(config["start-row"])
    end_row = int(config["end-row"])
    ncols = int(grid_template_metadata["ncols"])
    setup_id_to_data = defaultdict(lambda: {
        "start_row": start_row,
        "end_row": end_row,
        "nrows": end_row - start_row + 1 if start_row > 0 and end_row >= start_row else int(grid_template_metadata["nrows"]),
        "ncols": ncols,
        "header": header,
        ...
        if msg["type"] == "finish":
            ...
            while data["datacell-count"][data["next-row"]] == 0:
                ...
                and ((data["end_row"] < 0 and data["next-row"] > data["nrows"]-1) \
                    ...
                    if config["no-of-setups"] and process_message.setup_count >= int(config["no-of-setups"]):
